...
burn_projects <- c(2291, 282, 662, 672, 1092, 2021, 2878, 3577, 388, 1251, 1252, 1253, 1935, 2879, 2195, 2317, 2286, 2300, 2640, 2282, 3127, 4091, 3938, 3736, 3249, 2804)
...
Consistent site naming is critical for linking repeat surveys across years and for ensuring data load cleanly into systems such as WildTrax. Each site should use a stable naming scheme that persists over time, even when surveys are repeated annually or seasonally. A format such as `Region_Code_SiteNumber_NorthGrid` allows records from different years to be reliably joined and avoids the fragmented datasets that result from ad-hoc renaming.

Where possible, site names should incorporate hierarchical identifiers that move from broad to specific (for example, project or region followed by sub-site and point). This structure supports automated grouping and summarization without requiring manual reconciliation. To support long-term consistency, the naming schema and any abbreviations used should be documented in a short README or metadata dictionary that accompanies each dataset. Explicit documentation is especially important as field crews and analysts change over time.

### Location metadata

Although not every analysis requires full metadata, consistently recording a core set of location attributes substantially increases downstream utility and interoperability. At a minimum, this includes the latitude and longitude (decimal degrees) of each survey point or plot, along with the date and time of data collection. Information on who collected the data and the recorder or protocol used provides essential context for quality control and interpretation.

Environmental descriptors such as vegetation type, weather conditions, and notable pre-burn characteristics further improve interpretability. Together, these metadata make it much easier to integrate field observations with remote sensing products and to interpret burn severity and biological responses in a broader ecological context.

### Burn severity

For a field-usable and repeatable approach to assessing burn severity, the Composite Burn Index (CBI) provides a well-established framework. CBI is designed to characterize fire effects across vegetation and substrate layers in a way that is comparable among sites and directly compatible with satellite-derived burn severity products.

Assessments are conducted within a defined plot, typically on the order of 30 m in diameter to align with common satellite pixel sizes. Observations are made across five vertical strata: substrate (litter, duff, soil), herbaceous and low shrub layers, tall shrubs and saplings, intermediate trees, and large trees. Within each stratum, multiple attributes—such as vegetation mortality, soil exposure, and char height—are scored on a standardized 0–3 scale, where 0 represents unburned conditions and 3 indicates the highest severity. Scores are averaged within strata and then combined to produce a composite severity value for the plot.

To ensure consistency and transparency, field assessments should be recorded on standardized scoring sheets and supported by photographs taken from fixed viewpoints. In practice, field sheets should capture plot identifiers that align with the site naming convention, GPS coordinates, date and time, and observer names, followed by structured scoring for each vegetation and substrate layer. Additional observations, such as maximum char height, the presence of unburned refugia, or evidence of fire skips, provide valuable context for interpretation.

For operational use, composite scores can be grouped into simplified burn severity classes (unburned, low, moderate, high). These classes retain the ecological meaning of the underlying CBI scores while providing a clear, interpretable summary that can be readily linked to management objectives and remote sensing products.
