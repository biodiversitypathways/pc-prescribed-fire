...
prepost <- burns_fire |>
  ...
  group_by(location_id, year, sample_type) |>
  summarise(
    richness = n_distinct(species_code),
    .groups = "drop"
  ) %>%
  group_by(location_id, sample_type) %>%
...
prepost %>%
  select(location_id, `pre-burn`, `post-burn`) %>%
  pivot_longer(
  ...
  ggplot(aes(x = sample_type, y = value, group = location_id)) +
  geom_line(alpha = 0.4) +
  geom_point(size = 2) +
  stat_summary(
    fun = median,
