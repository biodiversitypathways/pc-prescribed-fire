...
Data were collected using passive autonomous recording units (ARUs; @Shonfield2017) at `r length(unique(burns$location))` locations (@fig-aru-monitoring-locations).
...
#| message: false
#| fig-align: center
#| fig-cap: Recording duration and method type collected from each national park.
...
Species richness overall did not change significantly following burns. Paired pre- and post-burn comparisons across locations showed no systematic increase or decrease in richness (Wilcoxon signed-rank test, *V* = 19.5, *p* = 0.44).
...
#| message: false
#| label: fig-paired
#| tbl-cap: Paired comparison of species richness before and after fire. Lines connect values from the same location, points show individual observations, and mean values are highlighted.
...
# Plot
burns_fire_paired %>%
  ggplot(aes(x = burn_status, y = richness, group = location_id)) +
  geom_line(alpha = 0.3) +
  geom_point() +
  theme_bw() +
...
ggplot(tsf_summary, aes(x = ysf, y = total_abundance, colour = organization)) +
  geom_point() +
  labs(
    x = "Years Since Fire",
    y = "Total Abundance",
...
comm_long <- burns_fire %>%
  inner_join(burns_fire_paired |> select(location_id, year, burn_status) |> distinct(), by = c("location_id" = "location_id", "year" = "year")) |>
  filter(burn_status %in% c("pre-burn", "post-burn")) %>%
  group_by(location_id, year, burn_status, veg_type, species_code) %>%
...
Consistent site naming is essential for linking repeat surveys across years and for ensuring data load cleanly into the WildTrax. Location names should follow a stable, documented convention that persists across seasons and years (e.g. EINP-MOS-3, AC-C2). This avoids fragmentation caused by ad-hoc renaming and allows records from different sampling periods to be reliably joined. Where possible, location names should use a hierarchical structure, moving from broad to specific (e.g., project or region → sub-site → point → treatment). This supports automated grouping, summarization, and analysis without manual reconciliation. The one condition however is not to indicate the sites by burn type, year or state after burn. This metadata would be accordingly joined afterwards to the location name as this would provide inconsistency within the WildTrax system. Nevertheless, each location in WildTrax has the ability to record spatial coordinates and Merging locations is possible. 
...
recs_table %>%
  kable(
    format = "html",
    caption = "Recommended ARU Recording Schedule Across Seasons and Daily Times"
  ) %>%
  kable_styling(
    full_width = FALSE,
    position = "left",
